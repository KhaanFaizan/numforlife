...
| D-013 | 08-10 | Use the PlenorHub **`shuyi`** channel (see §10.0) | 🔬 §10.0 | Only channel with content; also matches the registered KCC `client_id` | Identity + commerce resolve to one tenant | ✅ Accepted |
| D-014 | 08-10 | **Chinese only — no language switcher** | Project decision | Matches the approved demo; PRD's SEO keywords are all Chinese | Removes per-locale CMS schema, `/[locale]/` routing and translation workflow entirely — material simplification | ✅ Accepted |
| D-015 | 08-10 | **Gamification out of scope** — achievements (33), tasks (27), daily energy check-in, AI fortune indices | Project decision | Present in the app DB but absent from the PRD, therefore not contracted | Those tables are never read or written by the website | ✅ Accepted |
| D-016 | 08-10 | Reuse `yzn_phase3_event` for backend event tracking rather than building new infrastructure | 🔬 review §4A | Table already exists (`userid`, `event`, `feature`, `add_time`); satisfies PRD §16 "backend event tracking" and keeps website events in the same stream as app events | No new analytics backend; GA4 handles client-side | 💡 Proposed |
