      ...
      <div className="flex-1 overflow-y-auto px-8 py-8">
        ...
        <div className="grid gap-5 md:grid-cols-3">
          <AdminStatCard
            ...
            value={content.homepageBlocks.length}
            icon={<Layers className="h-5 w-5 text-black/60" />}
            accent="accent"
          />
          <AdminStatCard
            label="Feature Items"
            value={content.features.items.length}
            icon={<LayoutGrid className="h-5 w-5 text-black/60" />}
            accent="emerald"
          />
          <AdminStatCard
            label="Published Pages"
            value={content.pages.filter((p) => p.status === "published").length}
        ...
        <AdminCard className="mt-8" padding="lg">
          ...
          <div className="flex flex-wrap gap-2">
            {content.homepageBlocks.map((block, index) => (
              <span
                key={block.id}
